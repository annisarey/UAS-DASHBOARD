...
dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("Analisis Spasial", tabName = "spasial", icon = icon("map")),
  ...
  dashboardBody(
    ...
    tabItems(
      # BERANDA
      tabItem("beranda",
              h2("Selamat Datang di INUL Dashboard"),
              HTML("<br><b>INUL DASHBOARD</b> <i>(Instrumen Nasional untuk Uji Lanjutan)</i> menyediakan serangkaian alat untuk analisis statistik, mulai dari manajemen data hingga pemodelan regresi, berdasarkan data kerentanan sosial di Indonesia.<br>"),
              p("Telusuri data, uji hipotesis, dan temukan pola penting dalam kerentanan sosial Indonesia."),
              hr(),
              box(
              ...
              box(
                title = "Variabel Utama", width = 12, collapsible = TRUE, 
      ...
      tabItem("spasial",
              h2("Analisis Spasial Kerentanan Sosial"),
              tabsetPanel(
                id = "spasial_tabs",
                type = "tabs",
                tabPanel("Distribusi Kerentanan",
                         br(),
                         fluidRow(
                               box(width = 4, title = "Pengaturan Visualisasi", class = "softpink-box",
                               selectInput("spasial_var", "Pilih Indikator Kerentanan:", choices = NULL),
                               selectInput("spasial_palette", "Pilih Skema Warna:", 
                                          choices = c("Reds" = "Reds", "Blues" = "Blues", "Greens" = "Greens", 
                                                     "Purples" = "Purples", "Oranges" = "Oranges")),
                               numericInput("spasial_bins", "Jumlah Kelas Warna:", value = 5, min = 3, max = 9),
                               actionButton("run_spasial_map", "Tampilkan Visualisasi", icon = icon("chart-bar"), class = "btn-softpink btn-block")
                         ),
                               box(width = 8, title = "Visualisasi Kerentanan Sosial", class = "softpink-box",
                               uiOutput("status_spasial"),
                               leafletOutput("peta_kerentanan_leaflet", height = "600px")
                           )
                         ),
                         fluidRow(
                               box(width = 12, title = "Statistik Spasial", class = "softpink-box",
                               verbatimTextOutput("statistik_spasial"),
                               uiOutput("interpretasi_spasial")
                           )
                         )
                ),
                tabPanel("Cluster Wilayah",
                         br(),
                         fluidRow(
                           box(width = 4, title = "Pengaturan Cluster", class = "softpink-box",
                               selectInput("cluster_var", "Pilih Indikator untuk Clustering:", choices = NULL),
                               numericInput("cluster_k", "Jumlah Cluster:", value = 3, min = 2, max = 8),
                               selectInput("cluster_method", "Metode Clustering:", 
                                          choices = c("K-means" = "kmeans", "Hierarchical" = "hclust")),
                               actionButton("run_cluster", "Analisis Cluster", icon = icon("sitemap"), class = "btn-softpink btn-block")
                           ),
                                                       box(width = 8, title = "Visualisasi Cluster", class = "softpink-box",
                                uiOutput("status_cluster"),
                               leafletOutput("peta_cluster", height = "600px")
                           )
                         ),
                         fluidRow(
                           box(width = 12, title = "Hasil Analisis Cluster", class = "softpink-box",
                               verbatimTextOutput("hasil_cluster"),
                               uiOutput("interpretasi_cluster")
                           )
                         )
                ),
                tabPanel("Analisis Outlier",
                         br(),
                         fluidRow(
                           box(width = 4, title = "Pengaturan Hotspot", class = "softpink-box",
                               selectInput("hotspot_var", "Pilih Indikator:", choices = NULL),
                               selectInput("hotspot_method", "Metode Analisis:", 
                                          choices = c("Local Moran's I" = "moran", "Getis-Ord Gi*" = "getis")),
                               numericInput("hotspot_threshold", "Threshold Signifikansi:", value = 0.05, min = 0.01, max = 0.1, step = 0.01),
                                                               actionButton("run_hotspot", "Analisis Hotspot", icon = icon("fire"), class = "btn-softpink btn-block")
                           ),
                                                       box(width = 8, title = "Visualisasi Outlier", class = "softpink-box",
                                uiOutput("status_hotspot"),
                               leafletOutput("peta_hotspot", height = "600px")
                           )
                         ),
                         fluidRow(
                                                       box(width = 12, title = "Hasil Analisis Hotspot", class = "softpink-box",
                               verbatimTextOutput("hasil_hotspot"),
                               uiOutput("interpretasi_hotspot")
                           )
                         )
                )
              ),
              fluidRow(
                box(width = 12, title = "Unduh Hasil Analisis Spasial", class = "softpink-box",
                    downloadButton("unduh_spasial_plot", "Unduh Peta (PNG)", class = "btn-softpink"),
                    hr(),
                    radioButtons("format_spasial_report", "Pilih Format Laporan:", choices = c("PDF", "Word"), inline = TRUE),
                    downloadButton("unduh_spasial_report", "Unduh Laporan", class = "btn-softpink")
                )
              )
      ),
